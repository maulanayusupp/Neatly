...
- [x] **Per-tool FAQ + `FAQPage` JSON-LD** — `ToolFaq.vue` + `app/utils/faqs.ts`,
      now covering every tool.
- [x] **Related-tools internal links** — `RelatedTools.vue` on every tool page.
- [x] **`SoftwareApplication` + `BreadcrumbList` JSON-LD** per tool (`app.vue`).
